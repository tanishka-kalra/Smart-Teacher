...
def c_compile(file):
    with open("error.txt","w") as error_file:
        subprocess.run(["gcc",file,"-o",f"{file[0:-2]}.out"], stderr=error_file)
    ...
    if len(error)==0:
        ...
        with open("output.txt","w") as output_file:
            subprocess.run([f"./{file[0:-2]}.out"],stdout=output_file)
...
def py_compile(file):
    with open("error.txt","w") as error_file:
        with open("output.txt","w") as output_file:
            subprocess.run(["python3",file],stderr=error_file,stdout=output_file)
    err_f=open("error.txt","r")
    error=err_f.read()
    err_f.close() 
    if len(error)==0:
